...
function initAudioPlayer() {
	...
	playBtn.addEventListener('click', () => {
		if (audio.paused) {
			audio.play();
			playBtn.src = playBtn.dataset.pauseIcon || "{% static 'img/Pause.png' %}";
			rafId = requestAnimationFrame(updateMaskSmooth);
		} else {
			audio.pause();
			playBtn.src = playBtn.dataset.playIcon || "{% static 'img/Play.png' %}";
...
function initHiranandaniAnimation() {
	...
	hTL
		...
		.fromTo(
			...
			}, // runs after previous timeline step
		)
		.fromTo(
		...
		.fromTo(
			'.hiranandani-text-box .btn-fill-center',
			{ opacity: 0, '--reveal': 0 },
			{
				opacity: 1,
				'--reveal': 1.08,
...
// ============================================
// NATURAL SCROLL (FIXED)
// ============================================

// Remove aggressive custom wheel scrolling
// Keep only lightweight load handling
